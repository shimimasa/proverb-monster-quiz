...
interface GameContextType {
  ...
  learningHistoryManager: typeof learningHistoryManager;
...
export const GameProvider: React.FC<GameProviderProps> = ({ children }) => {
  ...
  const loadNewQuestion = async (type?: ContentType) => {
    ...
    try {
      ...
      if (type) {
        ...
        contentItem = contentManager.getRandomQuestion(selectedType, settings.difficulty);
      } else if (enabledTypes.length > 1 && settings.contentTypeWeights) {
        // 混合モード
        contentItem = contentManager.getRandomQuestionMixed(
          enabledTypes,
      ...
      const choices = contentManager.generateChoices(contentItem, contentItem.type);
  ...
  const submitAnswer = (choiceIndex: number) => {
    ...
    const levelUp = progressManager.updateProgress(quizResult.isCorrect, currentQuestion.contentItem.type);
    if (levelUp) {
