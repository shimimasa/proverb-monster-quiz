...
import { FaUpload, FaDownload, FaFileImport, FaFileExport, FaFileCsv } from 'react-icons/fa';
...
interface ImportExportProps {
  ...
  onExportCSV: (contents: ContentItem[], includeId: boolean) => string;
  allContents: ContentItem[];
}

export const ImportExport: React.FC<ImportExportProps> = ({ onImport, onExport, onExportCSV, allContents }) => {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-green-50 p-4 rounded-lg">
        ...
        <div className="space-y-4">
          {/* JSONエクスポート */}
          <div>
            <h4 className="font-medium mb-2">JSONエクスポート（カスタムコンテンツのみ）</h4>
            <p className="text-sm text-gray-600 mb-2">
              管理画面で追加したカスタムコンテンツをJSON形式でエクスポートできます。
            </p>
            <div className="flex space-x-3">
              <button
          ...
          {/* CSVエクスポート */}
          <div>
            <h4 className="font-medium mb-2">CSVエクスポート（全コンテンツ）</h4>
            <p className="text-sm text-gray-600 mb-2">
              オリジナルとカスタムを含む全コンテンツをCSV形式でエクスポートできます。
            </p>
            <div className="flex space-x-3">
              <button
                onClick={() => {
                  const filteredContents = allContents.filter(c => c.type === selectedType);
                  const csv = onExportCSV(filteredContents, true);
                  downloadFile(csv, `${selectedType}_all.csv`, 'text/csv;charset=utf-8');
                }}
                className="flex items-center space-x-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
              >
                <FaFileCsv />
                <span>{getTypeLabel(selectedType)}をCSVエクスポート</span>
              </button>
              
              <button
                onClick={() => {
                  const csv = onExportCSV(allContents, true);
                  downloadFile(csv, 'all_contents.csv', 'text/csv;charset=utf-8');
                }}
                className="flex items-center space-x-2 px-4 py-2 bg-white border border-blue-300 text-blue-700 rounded-lg hover:bg-blue-50 transition-colors"
              >
                <FaFileCsv />
                <span>全タイプをCSVエクスポート</span>
              </button>
            </div>
          </div>
        </div>
