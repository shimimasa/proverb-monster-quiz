...
export class AdminManager {
   ...
   * コンテンツのCSVエクスポート
   */
  public exportToCSV(contents: ContentItem[], includeId: boolean = false): string {
    // ヘッダー行
    const headers = includeId 
      ? ['ID', 'テキスト', '読み方', '意味', '難易度', '例文', 'タイプ']
      : ['テキスト', '読み方', '意味', '難易度', '例文', 'タイプ'];
    ...
    contents.forEach(content => {
      const row = includeId
        ? [
            content.id.toString(),
            this.escapeCSV(content.text),
            this.escapeCSV(content.reading),
            this.escapeCSV(content.meaning),
            content.difficulty,
            this.escapeCSV(content.example_sentence || ''),
            content.type
          ]
        : [
            this.escapeCSV(content.text),
