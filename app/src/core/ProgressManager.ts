...
export class ProgressManager {
  ...
  private contentTypeCorrect: Map<ContentType, number> = new Map();
  ...
  updateProgress(isCorrect: boolean, contentType?: ContentType): LevelUpResult | null {
    ...
    if (contentType) {
      this.updateContentTypeTracking(contentType, isCorrect);
    }
  ...
  getDetailedStats(): {
    ...
    this.contentTypeCount.forEach((count, type) => {
      if (byContentType[type]) {
        ...
        byContentType[type].correct = this.contentTypeCorrect.get(type) || 0;
      }
    });
  ...
  private updateContentTypeTracking(type: ContentType, isCorrect: boolean): void {
    const count = this.contentTypeCount.get(type) || 0;
    this.contentTypeCount.set(type, count + 1);
    
    if (isCorrect) {
      const currentCorrect = this.contentTypeCorrect.get(type) || 0;
      this.contentTypeCorrect.set(type, currentCorrect + 1);
    }
  }

  private loadContentTypeStats(): void {
    const stats = this.storageManager.get('contentTypeStats');
    if (stats) {
      try {
        const parsed = JSON.parse(stats);
        this.contentTypeCount = new Map(Object.entries(parsed.counts || {}));
        this.contentTypeCorrect = new Map(Object.entries(parsed.correct || {}));
      } catch (error) {
        console.error('Failed to load content type stats:', error);
      }
    }
  }

  private saveContentTypeStats(): void {
    const stats = {
      counts: Object.fromEntries(this.contentTypeCount),
      correct: Object.fromEntries(this.contentTypeCorrect)
    };
    this.storageManager.set('contentTypeStats', JSON.stringify(stats));
  }
  ...
  private loadComboState(): ComboState {
    ...
    if (saved) {
      ...
      return {
        ...parsed,
        lastCorrectTime: parsed.lastCorrectTime || null
      };
    }
  ...
  private updateCombo(isCorrect: boolean): void {
    ...
    if (isCorrect) {
      ...
      if (this.comboState.lastCorrectTime) {
        const timeDiff = now.getTime() - new Date(this.comboState.lastCorrectTime).getTime();
      ...
      this.comboState.lastCorrectTime = now.toISOString();
